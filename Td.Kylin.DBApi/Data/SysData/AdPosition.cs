    ...
    public class AdPosition
    {
        ...
        static void InitAdPage()
        {
            ...
            foreach (var kv in TerminalDic)
            {
                ...
                {
                    pages = new[] { "启动页", "淘一淘首页", "附近购首页", "精品汇首页", "上门服务首页", "预约服务首页", "求职招聘首页", "发现首页" };
                }
        ...
        static void AddPositionForUserClient(IEnumerable<Ad_Page> pages)
        {
            ...
            foreach (var page in pages)
            {
                ...
                {
                    AddPosition(page.PageID, "顶部左侧按钮广告", "U01001_mallsjsc001", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                    AddPosition(page.PageID, "顶部右侧按钮广告", "U01001_mallsjsc002", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                    AddPosition(page.PageID, "中部第一栏横幅", "U01001_mallsjsc003", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                    AddPosition(page.PageID, "中部第二栏横幅", "U01001_mallsjsc004", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                }
                else if (page.PageName.Equals("精品汇首页"))
                {
                    AddPosition(page.PageID, "顶部左侧按钮广告", "U01001_mallqysc001", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                    AddPosition(page.PageID, "顶部右侧按钮广告", "U01001_mallqysc002", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                    AddPosition(page.PageID, "中部横幅", "U01001_mallqysc003", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                ...
                else if (page.PageName.Equals("发现首页"))
                {
                    ...
                    AddPosition(page.PageID, "中部左一按钮广告", "U01001_find002", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                    AddPosition(page.PageID, "中部右一按钮广告", "U01001_find003", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                    AddPosition(page.PageID, "中部右二按钮广告", "U01001_find004", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                    AddPosition(page.PageID, "中部右三按钮广告", "U01001_find005", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                    AddPosition(page.PageID, "中部横幅", "U01001_find006", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                    AddPosition(page.PageID, "底部左一按钮广告", "U01001_find007", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                    AddPosition(page.PageID, "底部右一按钮广告", "U01001_find008", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                    AddPosition(page.PageID, "底部右二按钮广告", "U01001_find009", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                    AddPosition(page.PageID, "底部右三按钮广告", "U01001_find010", EnumLibrary.ADDisplayType.SingleImage, "一般为单个图片广告，点击后进入相应的内容页", 1);
                }
            }
        }
